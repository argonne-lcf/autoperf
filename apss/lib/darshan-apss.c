...
void apss_runtime_initialize()
{
    ...
    sstopo_get_mycoords(&apss_runtime->group,
            ...
            &apss_runtime->blade,
            &apss_runtime->node);
    sprintf(rtr_rec_name, "darshan-apss-rtr-%d-%d-%d-%d",
            apss_runtime->group, apss_runtime->chassis, apss_runtime->slot, apss_runtime->blade);
        ...
        int shared_rec_count)
{
    ...
    if (my_rank == 0)
    {
        apss_runtime->header_record->appid = atoi((char*)getenv( APP_ID ));
    }
#if 1
    ...
    if (bitvec)
    {
        ...
        /* chassis */
        memset(bitvec, 0, bitlen);
        uchassis = apss_runtime->group * 8 + apss_runtime->chassis;
        idx = uchassis / bitcnt;
        ...
        /* slot */
        memset(bitvec, 0, bitlen);
        uslot = uchassis * 8 + apss_runtime->slot;
        idx = uslot / bitcnt;
        ...
        /* blade */
        memset(bitvec, 0, bitlen);
        ublade = uchassis * 8 + uslot * 8 + apss_runtime->blade;
        idx = ublade / bitcnt;
        ...
        if (my_rank == 0)
        {
            ...
            apss_runtime->header_record->nslots = slot_count;
    ...
    else
    {
        ...
        apss_runtime->header_record->nslots = 0;
     ...
     *  
     */ 
    color = (apss_runtime->group << (7+3)) + \
            (apss_runtime->chassis << (3+4)) + \
            (apss_runtime->slot << (3+1)) + \
            (apss_runtime->blade << 1) + apss_runtime->node;
#endif
#if 1
